...
class MRFOpen:
    ...
    def __enter__(self):
        ...
        log.info(f'{os.getpid()}: Successfully opened file: {self.loc}')
        return self.f
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.is_remote:
            self.r.close()
        self.f.close()
        log.info(f'{os.getpid()}: Successfully CLOSED file: {self.loc}')
    ...
    def in_network_items(
        ...
        for prefix, event, value in self.parser:
            ...
            if (prefix, event, value) == ('in_network', 'end_array', None):
                ...
                log.info(f"{num}: Rates found for {billing_code_type} {billing_code}")
                ...
                if (
                    ...
                    log.debug(f"{num}: Skipping {billing_code_type} {billing_code}: not in list")
                ...
                log.debug(f"{num}: Skipping {billing_code_type} {billing_code}: no providers")
                self.ffwd(('in_network.item', 'end_map', None))
                builder.value.pop()
                builder.containers.pop()
                builder.containers.pop()
...
async def flatten_mrf(
    ...
    with MRFOpen(loc) as f:
        ...
        if m.parser.current == ('', 'map_key', 'provider_references'):
            ...
            for item in m.in_network_items(npi_set, code_set, p_refs_map, num):
